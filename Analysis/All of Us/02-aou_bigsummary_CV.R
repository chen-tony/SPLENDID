...
# --input TRAIN_FOLD: matrix of training IDs for each CV fold
# --input PHENO
# --input COV
# --input RELATED
# --input ANCESTRY
...
print('folds')
train_fold = read.table(Sys.getenv('TRAIN_FOLD'), header=F)
...
print(' Z')
Z = data[ind_train,] %>% 
  select(any_of(c('age', 'female', paste0('PC', 1:20)))) %>% 
  data.matrix()
# Z_mu = colMeans(Z)
# Z_sd = apply(Z, 2, sd)
...
# parallel over each fold
registerDoParallel(cores=5)
out = foreach(fold = 1:5) %dopar% {
  fold_id = train_fold[,fold]
  ind_row2 = which((plink$fam$fam %in% data$FID) & 
                     (plink$fam$samp %in% data$IID) &
                     (plink$fam$samp %in% fold_id))
  ind_train2 = which(data$IID[ind_train] %in% fold_id)
  
  cat('plink and data IDs for fold ', fold, ': ', 
      all.equal(plink$fam$samp[ind_row2], data$IID[ind_train[ind_train2]]), '\n')
  
  res = residuals(lm(Y_std[ind_train2] ~ 0 + Z_std[ind_train2,]))
  
  test_summary = bigsummaries(BM=plink$genotypes$copy(code=c(0,1,2,rep(0,253))),
                              rowInd=ind_row2, colInd=ind_col,
                              E=E_std[ind_train2,],
                              Y=res)
