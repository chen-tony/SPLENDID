...
List cdfit_binomial_par(C macc,
  ...
  for (int l = 0; l < L; l++) {
    ...
    for (j=0; j<p; j++) {
      if (grouped[j]) {
        cutoff = std::sqrt(weights[j]*lambda0 / (XE_scale[j] + 2*weights[j]*lambda2)) + 0.5*weights[j]*lambda1 / (XE_scale[j] + 2*weights[j]*lambda2);
      } else {
        cutoff = std::sqrt(weights[j]*lambda0 / (X_scale[j] + 2*weights[j]*lambda2)) + 0.5*weights[j]*lambda1 / (X_scale[j] + 2*weights[j]*lambda2);
      }
    ...
    while (iter[l] < maxit) {
        ...
        for (j = 0; j < p; j++) {
          if (in_A[j]) {
            if (grouped[j]) {
              ...
              for (i = 0; i < n; i++) {
                z = macc(i,j) * r[i] / n / (XE_scale[j] + 2*weights[j]*lambda2);
                beta_tilde[0] += z;
                // beta_tilde[0] += macc(i,j) * r[i] / n / (XE_scale[j] + 2*weights[j]*lambda2);
                
                for (k=1; k<K+1; k++) {
                  beta_tilde[k] += z * E(i,k-1);
                  // beta_tilde[k] += macc(i,j) * E(i,k-1) * r[i] / n / (XE_scale[j] + 2*weights[j]*lambda2);
                }
              }
              ...
              if (beta_norm > cutoff) {
                // update beta
                beta.row(j) = beta_tilde * (1 - 0.5*weights[j]*lambda1 / beta_norm / (XE_scale[j] + 2*weights[j]*lambda2));
                
                L0 ++;
                L0_het ++; 
                L1 += beta_norm - 0.5*weights[j]*lambda1 / (XE_scale[j] + 2*weights[j]*lambda2);
                L2 += (beta_norm - 0.5*weights[j]*lambda1 / (XE_scale[j] + 2*weights[j]*lambda2)) * (beta_norm - 0.5*weights[j]*lambda1 / (XE_scale[j] + 2*weights[j]*lambda2));
              ...
              if (beta_norm > cutoff) {
                // update beta
                beta(j,0) = beta_tilde[0] * (1 - 0.5*weights[j]*lambda1 / beta_norm / (X_scale[j] + 2*weights[j]*lambda2));
      ...
      violations = check_strong_set(
        in_A, in_S, macc, E, score, X_scale, XE_scale, grouped, beta, weights[j]*lambda0, weights[j]*lambda1, weights[j]*lambda2, r);
...
List cdfit_binomial_parX(C macc,
  ...
  double metric = 0.0, metric_min = 1000000.0;
  int no_change = 0;
  
  arma::mat beta(p, K+1);
  arma::sp_mat best_beta(p, K+1);
  double best_lambda0=0.0, best_lambda1=0.0, best_lambda2=0.0;
  // sp_mat::const_iterator beta_it;
  ...
  for (int l = 0; l < L; l++) {
    ...
    if (metric < metric_min) {
      ...
      for (m=0; m<M; m++) {
        best_alpha(m) = alpha[m];
      }
    }
    
    // check dfmax
    if (nb_active[l] >= dfmax) {
      return List::create(_["beta"] = best_beta, 
                          _["alpha"] = best_alpha, 
