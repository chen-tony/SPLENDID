...
namespace gg_bigstatsr { namespace big_gglassoLin {

#include <gg_utils.hpp>

using namespace gg_bigstatsr::big_gglassoUtils;


// Gaussian loss
double COPY_gLoss(const arma::vec& r) {
  // Rcout << "gg_linear.hpp COPY_gLoss \n";
  return arma::dot(r, r);
}

// Coordinate descent for gaussian models L0L1
template <class C>
List COPY_cdfit_gaussian_hsr(C macc,
                              ...
                              const arma::mat& Z_val,
                              const arma::mat& E_val,
  ...
  // Rcout << "gg_linear.hpp COPY_cdfit_gaussian_hsr \n";
  ...
  double metric = 0.0, metric_min = 0.0;
  int no_change = 0;
  ...
  for (int l = 0; l < L; l++) {
    ...
    // objective
    loss[l] = COPY_gLoss(r) / n + lambda0*L0 + lambda1*L1 + lambda2*L2;
    nb_active[l] = L0;
    ...
    // prediction
    pred_val = predict(macc_val, beta, alpha, Z_val, E_val, grouped, in_A);
    metric = COPY_gLoss(pred_val - y_val) / n;
    // metric = arma::cor(pred_val, y_val);
    metrics[l] = metric;
    if (metric < metric_min) {
      metric_min = metric;
      no_change = 0;
    }
    for (i=0; i<n_val; i++) pred(i,l) = pred_val[i];
    ...
    Rcout << iter[l] << ", " << nb_checks[l] << ", " << nb_active[l] << ", " << metric << "\n";
...
// Coordinate descent for gaussian models L0L1 (no validation data)
template <class C>
List COPY_cdfit_gaussian_hsr0(C macc,
                             const arma::vec& y,
                             const arma::mat& Z,
                             const arma::mat& E,
                             const arma::mat& lambda,
  ...
  for (int l = 0; l < L; l++) {
    ...
    Rcout << iter[l] << "iters, " << L0 << " active \n";
    
    // objective
    metrics[l] = COPY_gLoss(r) / n;
...
// Coordinate descent for gaussian models L0L1 (no validation data)
template <class C>
List COPY_cdfit_gaussian_hsr0_2(C macc,
                              const arma::vec& y,
                              const arma::mat& Z,
                              const arma::mat& E,
                              const arma::mat& lambda,
                              const arma::vec& Z2,
                              const arma::mat& XE_scale,
                              const arma::mat& XEY,
                              const Rcpp::LogicalVector& grouped,
                              double tol,
                              int maxit,
  ...
  double beta_tilde=0.0, beta_j=0.0, dbeta=0.0;
  double L1 = 0.0;
  double L2 =0.0;
  int L0 = 0, L0_het=0;
  ...
  Rcpp::LogicalMatrix in_A(p, K+1); // ever-active set
  Rcpp::LogicalMatrix in_S(p, K+1); // strong set
  int n_strong=0, n_strong_het=0;
  ...
  double score0=0.0, scale0=0.0;
  arma::mat score = XEY / XE_scale; // score for strong set
  for (j=0; j<p; j++) {
    score(j,0) = std::fabs(XEY(j,0)) / XE_scale(j,0);
    
    if (grouped[j]) {
      for (k=1; k<K+1; k++) score(j,k) = std::fabs(XEY(j,k)) / XE_scale(j,k);
    }
  }
  ...
  for (int l = 0; l < L; l++) {
    ...
    n_strong_het = 0;
    for (j=0; j<p; j++) {
      // check if main effects are strong
      ...
      // check if interactions are strong
      if (grouped[j]) {
        for (k=1; k<K+1; k++) {
    ...
    Rcout << n_strong << "+" << n_strong_het << " strong, ";
    ...
    while (iter[l] < maxit) {
      while (iter[l] < maxit) {
        ...
        for (j = 0; j < p; j++) {
          ...
          if (in_A(j,0)) {
            ...
            beta_tilde = beta(j,0);
            for (i = 0; i < n; i++) {
              beta_tilde += macc(i,j) * r[i] / n / XE_scale(j,0);
            }
            
            score(j,0) = std::fabs(beta_tilde);
          
            // thresholding
            if (score(j,0) > cutoff) {
              // Rcout << "c.";
              // update beta
              beta(j,0) = beta_tilde * (1 - 0.5*lambda1 / score(j,0) / (XE_scale(j,0) + 2*lambda2));
              L0 ++;
              L1 += std::fabs(beta(j,0));
              L2 += beta(j,0) * beta(j,0);
              
              // difference in beta
              dbeta = beta(j,0) - beta_j;
              max_dbeta = std::fabs(dbeta);
            ...
            max_update = std::max(max_update, std::fabs(dbeta));
            
            // update residuals
            // Rcout << "[res]";
            if (std::fabs(dbeta) > 1e-20) {
              for (i=0; i<n; i++) {
                r(i) -= macc(i,j) * dbeta;
              }
            }
          ...
          if (grouped[j]) {
            for (k=1; k<K+1; k++) {
              if (in_A(j,k)) {
                ...
                beta_tilde = beta(j,k);
                for (i = 0; i < n; i++) {
                  beta_tilde += macc(i,j) * E(i,k-1) * r[i] / n / XE_scale(j,k);
                }
                
                score(j,k) = std::fabs(beta_tilde);
                
                // thresholding
                if (score(j,k) > cutoff) {
                  // Rcout << "c.";
                  // update beta
                  beta(j,k) = beta_tilde * (1 - 0.5*lambda1 / score(j,k) / (XE_scale(j,k) + 2*lambda2));
                  L0_het ++;
                  L1 += std::fabs(beta(j,k));
                  L2 += beta(j,k) * beta(j,k);
                  
                  // difference in beta
                  dbeta = beta(j,k) - beta_j;
                  max_dbeta = std::fabs(dbeta);
      ...
      // Scan for violations in strong set
      // Rcout << "[v.]";
      violations = COPY_check_strong_set2(
        in_A, in_S, macc, E, score, XE_scale, grouped, beta, lambda0, lambda1, lambda2, r);
      if (violations == 0 || check > ncheck) {
        break;
      } else {
        check++;
    ...
    Rcout << iter[l] << "iters, " << L0 << " active, " << L0_het << " interact \n";
    
    // objective
    metrics[l] = COPY_gLoss(r) / n;
    loss[l] = metrics[l] + lambda0*(L0+L0_het) + lambda1*L1 + lambda2*L2;
    nb_active[l] = L0;
    nb_interact[l] = L0_het;
    nb_checks[l] = check;
    nb_candidate[l] = n_strong;
    ...
    //// SNP effects
    for (j=0; j<p; j++) {
      if (in_A(j,0)) out_beta(j, (K+1)*l) = beta(j,0);
      if (grouped[j]) {
        for (k=1; k<K+1; k++) {
          if (in_A(j,k)) {
            for (k=1; k<K+1; k++) {
